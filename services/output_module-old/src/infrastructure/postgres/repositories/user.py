...
class UserDBGateWay:
    ...
    async def get_user_by_id(self, user_id: int) -> UserDTO:
        ...
        return UserDTO.model_validate(user.as_dict())
    ...
    async def get_user_with_password_by_login(self, login: str) -> Optional[tuple[UserDTO, str]]:
        ...
        if user is None:
            return None
        return UserDTO.model_validate(user.as_dict()), user.password_hash
    ...
    async def create_user(
        ...
        new_user = User(login=login, password_hash=password_hash, name=name, role=role.value)
        self.session.add(new_user)
        await self.session.flush()
        await self.session.refresh(new_user)
        return UserDTO.model_validate(new_user.as_dict())
    ...
    async def get_users_with_filters(self, filters: UserFilterDTO) -> tuple[list[UserDTO], int]:
        ...
        return [UserDTO.model_validate(user.as_dict()) for user in users], total
