...
const entity = {
  ...
  description:
    'Энергетический холдинг, специализирующийся на ветровой генерации и инфраструктуре устойчивой энергетики.',
};
...
const relatedEntities: LinkEntity[] = [
  ...
  <section class="profile">
    ...
    <section class="hero">
      <div>
        ...
        <h1>{{ entity.name }}</h1>
        <p class="jurisdiction">{{ entity.jurisdiction }}</p>
      </div>
      <p class="description">
        {{ entity.description }}
      </p>
    ...
    <section class="grid">
      <article class="card identifiers">
        ...
        <div class="registry">
          <p><strong>Юр. адрес:</strong> {{ registryInfo.address }}</p>
          <p><strong>Реестр:</strong> {{ registryInfo.registry }}</p>
          <p><strong>Основана:</strong> {{ registryInfo.founded }}</p>
        </div>
        <p class="links">
          Связан с:
          <a v-for="name in relatedSummary" :key="name" href="#">{{ name }}</a>
        </p>
      </article>
      ...
      <article class="card related">
        ...
        <ul>
          <li v-for="item in relatedEntities" :key="item.id">
            <div>
    ...
    <section class="mentions card">
      ...
      <div class="chart">
        <div v-for="point in mentions" :key="point.week" class="bar">
    ...
    <section class="news card">
      ...
      <div class="news-list">
        <article v-for="news in newsCards" :key="news.id" class="news-card">
          <div class="news-meta">
          ...
          <h3>{{ news.title }}</h3>
          <p>{{ news.summary }}</p>
          <button class="inline-link">Перейти к новости</button>
        </article>
      </div>
    </section>
